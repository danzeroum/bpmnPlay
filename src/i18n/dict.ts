...
export const DICT = {
  ...
  // C8 — Interop OMG (certifyXml + matriz CONFORMANCE + passthrough zeebe + CLI)
  'run.c8.intro': { pt: 'Importe 2 arquivos reais (Camunda/bpmn.io), certifique a classe OMG, veja a matriz CONFORMANCE viva e prove o passthrough zeebe:* com Δ nomeado.', en: 'Import 2 real files (Camunda/bpmn.io), certify the OMG class, see the live CONFORMANCE matrix and prove the zeebe:* passthrough with named Δ.' },
  'run.c8.error': { pt: 'Não consegui carregar/certificar o arquivo.', en: 'Could not load/certify the file.' },
  'run.c8.import.h': { pt: 'Importe e certifique', en: 'Import and certify' },
  'run.c8.import.sub': { pt: 'Dois exports representativos (namespace OMG). certifyXml dá a classe atingida — honesta, sobre o comportamento declarado na matriz.', en: 'Two representative exports (OMG namespace). certifyXml reports the achieved class — honest, over the behavior declared in the matrix.' },
  'run.c8.class.none': { pt: 'não certifica', en: 'no class' },
  'run.c8.roundtrip': { pt: 'round-trip', en: 'round-trip' },
  'run.c8.elements': { pt: 'elementos', en: 'elements' },
  'run.c8.warnings': { pt: 'avisos', en: 'warnings' },
  'run.c8.source': { pt: 'fonte', en: 'source' },
  'run.c8.pass.h': { pt: 'Passthrough zeebe:* com Δ nomeado', en: 'zeebe:* passthrough with named Δ' },
  'run.c8.pass.sub': { pt: 'Round-trip pelo mesmo converter: cada extensão estrangeira sobrevive por nome (nunca um blob opaco), e o diff modelo↔round-trip é vazio.', en: 'Round-trip through the same converter: each foreign extension survives by name (never an opaque blob), and the model↔round-trip diff is empty.' },
  'run.c8.pass.action': { pt: 'Provar passthrough (round-trip)', en: 'Prove passthrough (round-trip)' },
  'run.c8.pass.lossless': { pt: 'Round-trip lossless — as extensões preservadas por nome:', en: 'Round-trip lossless — extensions preserved by name:' },
  'run.c8.pass.lossy': { pt: 'Round-trip com diferença (inesperado).', en: 'Round-trip differs (unexpected).' },
  'run.c8.matrix.h': { pt: 'Matriz CONFORMANCE viva', en: 'Live CONFORMANCE matrix' },
  'run.c8.sources': { pt: 'Corpus externo (fonte · licença):', en: 'External corpus (source · license):' },
  'run.c8.cli.h': { pt: 'Reproduza no seu CI', en: 'Reproduce in your CI' },
  'run.c8.cli.sub': { pt: 'A mesma certificação, na linha de comando — porta de qualidade no seu pipeline.', en: 'The same certification, on the command line — a quality gate in your pipeline.' },
  'run.c8.cli.copy': { pt: 'Copiar', en: 'Copy' },
  'run.c8.cli.copied': { pt: 'Copiado ✓', en: 'Copied ✓' },
  'run.c8.s1.t': { pt: 'Importe o arquivo Camunda', en: 'Import the Camunda file' },
  'run.c8.s1.l': { pt: 'o badge de classe e os avisos do certifyXml', en: 'the class badge and certifyXml warnings' },
  'run.c8.s2.t': { pt: 'Importe o arquivo bpmn.io', en: 'Import the bpmn.io file' },
  'run.c8.s2.l': { pt: 'significado pleno — a classe atingida', en: 'full meaning — the achieved class' },
  'run.c8.s3.t': { pt: 'Leia a matriz CONFORMANCE', en: 'Read the CONFORMANCE matrix' },
  'run.c8.s3.l': { pt: 'a cobertura por classe e as fontes do corpus', en: 'the per-class coverage and corpus sources' },
  'run.c8.s4.t': { pt: 'Prove o passthrough', en: 'Prove the passthrough' },
  'run.c8.s4.l': { pt: 'zeebe:*/camunda:* preservados por nome no round-trip', en: 'zeebe:*/camunda:* preserved by name on round-trip' },
  'run.c8.s5.t': { pt: 'Leve para o seu CI', en: 'Take it to your CI' },
  'run.c8.s5.l': { pt: 'o comando bpmn-react certify --require analytic', en: 'the bpmn-react certify --require analytic command' },
  // C5 — Agente → humano (agentflow: agentTask governado + dry-run + escalação + LangGraph)
  'run.c5.intro': { pt: 'Um agentTask governado com o AgentStudio ao lado: simule em dry-run (trilha determinística, parada honesta), prove a escalação agente→humano como 1 undo, e exporte LangGraph com o subconjunto declarado.', en: 'A governed agentTask with the AgentStudio alongside: dry-run the simulation (deterministic trail, honest stop), prove the agent→human escalation as 1 undo, and export LangGraph with the declared subset.' },
  'run.c5.macro.h': { pt: 'agentTask governado + AgentStudio ao lado', en: 'Governed agentTask + AgentStudio alongside' },
  'run.c5.macro.sub': { pt: 'O «Agente de Pesquisa» (autonomia 2) no macro BPMN. Abra o AgentStudio para a revisão assinada (autor + timestamp + lifecycle) — a notação é neutra da lib.', en: 'The «Research Agent» (autonomy 2) in the BPMN macro. Open the AgentStudio for the signed review (author + timestamp + lifecycle) — the notation is the library’s neutral one.' },
  'run.c5.openStudio': { pt: 'Abrir AgentStudio', en: 'Open AgentStudio' },
  'run.c5.dry.h': { pt: 'Simule em dry-run', en: 'Dry-run the simulation' },
  'run.c5.dry.sub': { pt: '`simulate` do agentflow: a MESMA entrada dá a MESMA trilha (determinística) e para de forma honesta quando os retries esgotam — parada declarada, não travamento.', en: 'agentflow `simulate`: the SAME input yields the SAME trail (deterministic) and stops honestly when retries are exhausted — a declared stop, not a hang.' },
  ...
  'run.c5.esc.undoable': { pt: 'aplicado e desfeito por 1 comando', en: 'applied and undone by 1 command' },
  'run.c5.esc.nodes': { pt: 'nós', en: 'nodes' },
  'run.c5.export.h': { pt: 'Exporte LangGraph (subconjunto declarado)', en: 'Export LangGraph (declared subset)' },
  'run.c5.export.sub': { pt: '`exportLangGraph` traduz o agente para o formato LangGraph e DECLARA o que fica de fora (autonomia, decoradores, arestas de delegação) — subconjunto honesto, nunca perda silenciosa.', en: '`exportLangGraph` translates the agent to LangGraph and DECLARES what is left out (autonomy, decorators, delegate edges) — an honest subset, never silent loss.' },
  'run.c5.export.action': { pt: 'Exportar LangGraph', en: 'Export LangGraph' },
  'run.c5.s1.t': { pt: 'Simule em dry-run', en: 'Dry-run the simulation' },
  'run.c5.s1.l': { pt: 'a trilha determinística e a parada honesta', en: 'the deterministic trail and the honest stop' },
  'run.c5.s2.t': { pt: 'Prove a escalação', en: 'Prove the escalation' },
  'run.c5.s2.l': { pt: 'o boundary agente→humano como 1 comando undoable', en: 'the agent→human boundary as 1 undoable command' },
  'run.c5.s3.t': { pt: 'Exporte LangGraph', en: 'Export LangGraph' },
  'run.c5.s3.l': { pt: 'o subconjunto declarado nos avisos', en: 'the declared subset in the warnings' },
  'run.c5.s4.t': { pt: 'Abra o AgentStudio', en: 'Open the AgentStudio' },
  'run.c5.s4.l': { pt: 'a revisão assinada: autor, timestamp e lifecycle', en: 'the signed review: author, timestamp and lifecycle' },
  'home.footer.dev': {
